    ...
    const Quiz6 = () => {
        ...
        const questions= [
            {
                question:"Qu'est-ce qu'une MAPI ?",
                
                options: ['Un événement médical indésirable qui survient après une vaccination sans lien nécessaire avec le vaccin', 'Une réaction allergique grave uniquement', 'Une infection causée par le vaccin', 'Un symptôme bénin qui survient uniquement chez les adultes'],
                
              correctAnswer:'Un événement médical indésirable qui survient après une vaccination sans lien nécessaire avec le vaccin',
            },
            {
                question:'Quelle est la réaction MAPI la plus fréquente après la vaccination contre le paludisme?',
            
                options: [' Convulsions fébriles', 'Anaphylaxie', "Fièvre, irritabilité, douleur et gonflement au point d'injection", 'Paralysie'],
                correctAnswer:"Fièvre, irritabilité, douleur et gonflement au point d'injection",
    
            },
            {
                question:'Quel est le rôle des agents de santé dans la communication avec les parents en cas de MAPI ?',
                
                options: ["Minimiser l'importance des MAPI", ' Donner des informations incorrectes pour rassurer les parents', 'Rassurer les parents, expliquer que le vaccin est sûr et fournir des informations précises', 'Ignorer les préoccupations des parents'],
    
                correctAnswer:'Rassurer les parents, expliquer que le vaccin est sûr et fournir des informations précises',
            },
          
            {
                question:'Quelle est la procédure correcte pour signaler une MAPI?',
                
                options: ["Remplir le formulaire de notification et l'envoyer au superviseur", ' Ignorer la MAPI si elle semble mineure', ' Attendre la fin de la journée pour signaler toutes les MAPI en une seule fois', 'Informer uniquement les parents'],
    
                correctAnswer:"Remplir le formulaire de notification et l'envoyer au superviseur",
            },
            {
                question:'Comment doit réagir un agent de santé en cas de MAPI grave ?',
                
                options: ['Ignorer la situation et continuer la vaccination', ' Alerter immédiatement le superviseur et commencer le traitement approprié', "Noter l'incident pour une revue future",' Réprimander les parents pour ne pas avoir signalé plus tôt'],
                correctAnswer:' Alerter immédiatement le superviseur et commencer le traitement approprié',
    
            },
            {
                question:'Pourquoi est-il important de signaler toutes les MAPI, même mineures ?',
                
                options: ['Pour augmenter les statistiques de vaccination', "Pour s'assurer que toutes les réactions sont documentées et analysées pour la sécurité des malades", ' Pour recevoir des récompenses pour la notification',' Pour informer les médias'],
                
                correctAnswer:"Pour s'assurer que toutes les réactions sont documentées et analysées pour la sécurité des malades",
            }
          ];
        ...
        useEffect(() => {
            const checkCooldown = async () => {
                try {
                    const lastAttemptTime6 = await AsyncStorage.getItem('lastAttemptTime6');
                    if (lastAttemptTime6) {
                        const currentTime = new Date().getTime();
                        const timeDifference = currentTime - parseInt(lastAttemptTime6);
        ...
        const startCooldown = async () => {
            try {
                const currentTime = new Date().getTime();
                await AsyncStorage.setItem('lastAttemptTime6', JSON.stringify(currentTime));
                setIsCooldownActive(true);
            } catch (error) {
        ...
        const storeAnswers = async (answersArray, finalScore) => {
            try {
                await AsyncStorage.setItem('Quiz6Answers', JSON.stringify(answersArray));
